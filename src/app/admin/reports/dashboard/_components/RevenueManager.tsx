...
import { DollarSign, TrendingUp, Target, Plus, Trash2, Calendar, ChevronDown, ChevronUp } from "lucide-react";
...
export function RevenueManager({ revenue, onSave }: RevenueManagerProps) {
  ...
  // 프로젝트별 수익 데이터 초기화
  const projectRevenues = revenue.projectRevenues || (Object.keys(PROJECT_INFO) as Array<Exclude<ProjectType, "all">>).map((project) => ({
    project,
    currentRevenue: 0,
    targetRevenue: 0,
  ...
  const currentProjectRevenue = useMemo(() => {
    ...
    return projectRevenues.find((p) => p.project === selectedProject) || {
      currentRevenue: 0,
      targetRevenue: 0,
      monthlyData: months.map((month) => ({ month, current: 0, target: 0 })),
    };
  ...
  const progress = currentProjectRevenue.targetRevenue > 0
    ? Math.round((currentProjectRevenue.currentRevenue / currentProjectRevenue.targetRevenue) * 100)
    : 0;
  ...
  // 프로젝트별 수익 업데이트
  const updateProjectRevenue = (project: Exclude<ProjectType, "all">, field: "currentRevenue" | "targetRevenue", value: number) => {
  ...
  // 월별 수익 업데이트
  const updateMonthlyRevenue = (project: Exclude<ProjectType, "all">, monthIndex: number, field: "current" | "target", value: number) => {
    const updatedProjectRevenues = projectRevenues.map((p) => {
      if (p.project === project) {
        ...
        // 월별 합계로 전체 수익 자동 계산
        const newCurrent = field === "current"
    ...
    onSave({
      ...
      monthlyData: months.map((month, idx) => ({
        month,
        current: updatedProjectRevenues.reduce((sum, p) => sum + (p.monthlyData[idx]?.current || 0), 0),
        target: updatedProjectRevenues.reduce((sum, p) => sum + (p.monthlyData[idx]?.target || 0), 0),
      })),
    });
  };
        ...
        {selectedProject !== "all" && (
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="number"
                value={projectRevenues.find((p) => p.project === selectedProject)?.currentRevenue || ""}
                onChange={(e) => updateProjectRevenue(selectedProject as Exclude<ProjectType, "all">, "currentRevenue", Number(e.target.value) || 0)}
            ...
            <div>
              ...
              <input
                type="number"
                value={projectRevenues.find((p) => p.project === selectedProject)?.targetRevenue || ""}
                onChange={(e) => updateProjectRevenue(selectedProject as Exclude<ProjectType, "all">, "targetRevenue", Number(e.target.value) || 0)}
          ...
          <div className="bg-[#25262b] rounded-lg p-4 border border-[#373A40]">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-gray-200">
                {selectedProject === "all" ? "전체 달성률" : `${PROJECT_INFO[selectedProject as Exclude<ProjectType, "all">].label} 달성률`}
              </span>
              <span className={`text-lg font-bold ${
                progress >= 100 ? "text-emerald-400" :
                progress >= 70 ? "text-blue-400" :
                progress >= 30 ? "text-amber-400" : "text-red-400"
              }`}>
                {progress}%
              </span>
            </div>
        ...
        {selectedProject === "all" && (
          <div className="space-y-3">
            ...
            {(Object.keys(PROJECT_INFO) as Array<Exclude<ProjectType, "all">>).map((project) => {
              ...
              const projectProgress = projectData && projectData.targetRevenue > 0
                ...
                <div key={project} className="bg-[#25262b] rounded-lg p-3 border border-[#373A40]">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs text-[#909296]">
                      {formatCurrency(projectData?.currentRevenue || 0)} / {formatCurrency(projectData?.targetRevenue || 0)}
                    </span>
                  </div>
                  <div className="h-2 bg-[#373A40] rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        backgroundColor: info.color.includes("emerald") ? "#10b981" :
                                        info.color.includes("blue") ? "#3b82f6" : "#8b5cf6"
                      }}
                    />
                  </div>
                </div>
        ...
        {selectedProject !== "all" && (
          <div>
            ...
            >
              ...
              <span>월별 수익 입력</span>
              {showMonthlyInput ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
            </button>
            ...
            {showMonthlyInput && (
              <div className="mt-4 space-y-2 max-h-[300px] overflow-y-auto">
                {months.map((month, idx) => {
                  ...
                  const monthData = projectData?.monthlyData[idx] || { month, current: 0, target: 0 };
                  ...
                  return (
                    <div key={month} className="grid grid-cols-[60px_1fr_1fr] gap-2 items-center">
                      ...
                      <input
                        ...
                        onChange={(e) => updateMonthlyRevenue(selectedProject as Exclude<ProjectType, "all">, idx, "current", Number(e.target.value) || 0)}
                        placeholder="현재"
                      ...
                      <input
                        ...
                        onChange={(e) => updateMonthlyRevenue(selectedProject as Exclude<ProjectType, "all">, idx, "target", Number(e.target.value) || 0)}
                        placeholder="목표"
                        className="h-8 px-2 bg-[#25262b] border border-[#373A40] rounded text-xs text-white placeholder-[#5c5f66] focus:outline-none focus:border-brand-primary"
                      />
                    </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-[#25262b] rounded-lg p-3 border border-[#373A40] text-center">
            ...
            <div className={`text-lg font-semibold ${
              currentProjectRevenue.targetRevenue - currentProjectRevenue.currentRevenue <= 0 ? "text-emerald-400" : "text-blue-400"
            }`}>
              {formatCurrency(Math.max(0, currentProjectRevenue.targetRevenue - currentProjectRevenue.currentRevenue))}
            </div>
          </div>
        </div>
