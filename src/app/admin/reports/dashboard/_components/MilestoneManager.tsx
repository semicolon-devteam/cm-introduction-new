                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => setEditForm({ ...editForm, status: e.target.value as MilestoneItem["status"] })}
                          className="w-full h-9 px-3 bg-[#1a1b23] border border-[#373A40] rounded-md text-sm text-white focus:outline-none focus:border-brand-primary"
                        >
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`text-sm font-medium ${MILESTONE_STATUS[milestone.status].color}`}>
                          {milestone.title || "(제목 없음)"}
                        </span>
                        <span className={`px-2 py-0.5 text-xs rounded ${MILESTONE_STATUS[milestone.status].bg} ${MILESTONE_STATUS[milestone.status].color}`}>
                      ...
                      {milestone.description && (
                        <p className="mt-1 text-xs text-[#5c5f66] truncate">{milestone.description}</p>
                      )}
                    </div>
