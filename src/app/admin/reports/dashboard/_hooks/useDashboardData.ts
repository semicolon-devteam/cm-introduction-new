...
function calculateKPIs(data: DashboardData): KPIMetric[] {
  ...
  return [
    ...
    },
    {
      ...
      target: 100,
      unit: "%",
      trend: achievedRate >= 70 ? "up" : achievedRate >= 30 ? "neutral" : "down",
      color: "emerald",
    },
    {
      id: "revenue-progress",
      label: "매출 진행률",
      value: data.revenue.currentRevenue,
      target: data.revenue.targetRevenue,
      unit: "원",
    ...
    },
    {
      ...
      value: openIssues,
      unit: "건",
      trend: openIssues > 5 ? "down" : "neutral",
      color: "red",
    },
  ];
}
...
export function useDashboardData(period: PeriodFilter = "month") {
  ...
  const refreshIssues = useCallback(async () => {
    try {
      ...
      const response = await fetch("/api/admin/reports/dashboard?refresh=true");
      if (response.ok) {
        const result = await response.json();
