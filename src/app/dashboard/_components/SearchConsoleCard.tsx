...
interface QueryData extends SearchConsoleMetric {
  query: string;
}
interface PageData extends SearchConsoleMetric {
  page: string;
}
interface DeviceData extends SearchConsoleMetric {
  device: string;
}
interface DailyData extends SearchConsoleMetric {
  date: string;
}
      ...
      <div className={`bg-[#1a1b23] rounded-lg border border-[#373A40] p-5 ${className}`}>
        ...
        <div className="flex flex-col items-center justify-center py-8 text-center">
          ...
          <p className="text-xs text-[#5c5f66] max-w-sm">
            .env.local에 SEARCH_CONSOLE_SITE_URL 환경 변수를 설정하고, Service Account를 Search
            Console 사용자로 추가하세요
          </p>
          <a
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          <div className="bg-[#25262b] rounded-lg p-3">
            ...
            <div className="flex items-baseline gap-2">
              ...
              {overview?.change && (
                <span
                  className={`text-xs flex items-center gap-0.5 ${getTrendColor(overview.change.clicks)}`}
                >
          ...
          <div className="bg-[#25262b] rounded-lg p-3">
            ...
            <div className="flex items-baseline gap-2">
              ...
              {overview?.change && (
                <span
                  className={`text-xs flex items-center gap-0.5 ${getTrendColor(overview.change.impressions)}`}
                >
          ...
          <div className="bg-[#25262b] rounded-lg p-3">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-xl font-bold text-emerald-400">
                {overview?.current.ctr || 0}%
              </span>
              {overview?.change && (
                <span
                  className={`text-xs flex items-center gap-0.5 ${getTrendColor(overview.change.ctr)}`}
                >
          ...
          <div className="bg-[#25262b] rounded-lg p-3">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-xl font-bold text-white">
                {overview?.current.position || 0}
              </span>
              {overview?.change && (
                <span
                  className={`text-xs flex items-center gap-0.5 ${getTrendColor(overview.change.position)}`}
                >
              ...
              {activeTab === "queries"
                ? data.topQueries?.slice(0, 10).map((item, idx) => (
                    <tr key={idx} className="border-b border-[#373A40]/50 last:border-b-0">
                      ...
                      <td className="p-2 text-right text-[#909296]">
              ...
              <div className="bg-[#25262b] rounded-lg p-3">
                ...
                <div className="space-y-2">
                  {data.deviceBreakdown.map((device, idx) => {
                    ...
                    const percentage =
                      totalClicks > 0 ? Math.round((device.clicks / totalClicks) * 100) : 0;
