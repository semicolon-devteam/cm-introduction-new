...
import { Plus, Trash2, Edit2, Check, X, Server, AlertTriangle, Activity, Wrench } from "lucide-react";
...
export function OperationsManager({ operations, onSave }: OperationsManagerProps) {
  ...
  const cancelIncident = () => {
    if (incidentForm && !incidentForm.description) {
      onSave({ ...operations, incidents: operations.incidents.filter((i) => i.id !== incidentForm.id) });
    }
    setEditingIncident(null);
    setIncidentForm(null);
  };
  ...
  const cancelInfra = () => {
    if (infraForm && !infraForm.description) {
      onSave({ ...operations, infraChanges: operations.infraChanges.filter((c) => c.id !== infraForm.id) });
    }
    setEditingInfra(null);
    setInfraForm(null);
  };
        ...
        <div className="p-5">
          <div className="flex gap-2">
            {(Object.keys(SERVICE_STATUS) as Array<keyof typeof SERVICE_STATUS>).map((status) => (
              ...
              >
                <div className={`text-sm font-medium ${operations.serviceStatus === status ? "text-white" : SERVICE_STATUS[status].color}`}>
        ...
        <div className="p-5 grid grid-cols-2 md:grid-cols-4 gap-3">
          <div>
            ...
            <input
              ...
              onChange={(e) => onSave({ ...operations, metrics: { ...operations.metrics, uptime: e.target.value } })}
              placeholder="99.9"
          ...
          <div>
            <label className="block text-xs font-medium text-[#909296] mb-1.5">응답 시간 (ms)</label>
            <input
              type="text"
              value={operations.metrics.responseTime}
              onChange={(e) => onSave({ ...operations, metrics: { ...operations.metrics, responseTime: e.target.value } })}
              placeholder="150"
              className="w-full h-9 px-3 bg-[#25262b] border border-[#373A40] rounded-md text-sm text-white placeholder-[#5c5f66] focus:outline-none focus:border-brand-primary"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => onSave({ ...operations, metrics: { ...operations.metrics, errorRate: e.target.value } })}
              placeholder="0.1"
              className="w-full h-9 px-3 bg-[#25262b] border border-[#373A40] rounded-md text-sm text-white placeholder-[#5c5f66] focus:outline-none focus:border-brand-primary"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => onSave({ ...operations, metrics: { ...operations.metrics, activeUsers: e.target.value } })}
              placeholder="1250"
              className="w-full h-9 px-3 bg-[#25262b] border border-[#373A40] rounded-md text-sm text-white placeholder-[#5c5f66] focus:outline-none focus:border-brand-primary"
            />
          </div>
          ...
          {operations.incidents.length === 0 ? (
            ...
            <div className="space-y-2">
              {operations.incidents.map((incident) => (
                <div key={incident.id} className="bg-[#25262b] rounded-lg border border-[#373A40] p-3">
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <select
                          ...
                          onChange={(e) => setIncidentForm({ ...incidentForm, status: e.target.value as IncidentItem["status"] })}
                          className="h-9 px-3 bg-[#1a1b23] border border-[#373A40] rounded-md text-sm text-white focus:outline-none focus:border-brand-primary"
                        >
                      ...
                      <div className="flex justify-end gap-2">
                        <button onClick={cancelIncident} className="h-8 px-3 text-sm text-[#909296] hover:text-white rounded-md">
                          <X className="w-4 h-4" />
                        </button>
                        <button onClick={saveIncident} disabled={!incidentForm.description} className="h-8 px-3 text-sm text-white bg-brand-primary hover:bg-brand-primary/90 rounded-md disabled:opacity-50">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`px-2 py-0.5 text-xs rounded ${INCIDENT_STATUS[incident.status].color}`}>
                            {INCIDENT_STATUS[incident.status].label}
                          </span>
                        </div>
                        <p className="text-sm text-white mt-1">{incident.description || "(설명 없음)"}</p>
                      </div>
                      <div className="flex gap-1">
                        <button onClick={() => { setEditingIncident(incident.id); setIncidentForm({ ...incident }); }} className="p-1.5 text-[#909296] hover:text-white rounded-md">
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button onClick={() => deleteIncident(incident.id)} className="p-1.5 text-[#909296] hover:text-red-400 rounded-md">
      ...
      <div className="bg-[#1a1b23] rounded-lg border border-[#373A40] overflow-hidden">
        ...
        <div className="p-5">
          {operations.infraChanges.length === 0 ? (
            <p className="text-center text-[#5c5f66] text-sm py-4">등록된 인프라 변경 사항이 없습니다</p>
          ) : (
            <div className="space-y-2">
              {operations.infraChanges.map((change) => (
                <div key={change.id} className="bg-[#25262b] rounded-lg border border-[#373A40] p-3">
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <select
                          ...
                          onChange={(e) => setInfraForm({ ...infraForm, type: e.target.value as InfraChangeItem["type"] })}
                          className="h-9 px-3 bg-[#1a1b23] border border-[#373A40] rounded-md text-sm text-white focus:outline-none focus:border-brand-primary"
                        >
                      ...
                      />
                      <div className="flex justify-end gap-2">
                        <button onClick={cancelInfra} className="h-8 px-3 text-sm text-[#909296] hover:text-white rounded-md">
                          <X className="w-4 h-4" />
                        </button>
                        <button onClick={saveInfra} disabled={!infraForm.description} className="h-8 px-3 text-sm text-white bg-brand-primary hover:bg-brand-primary/90 rounded-md disabled:opacity-50">
                          <Check className="w-4 h-4" />
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`px-2 py-0.5 text-xs rounded ${INFRA_CHANGE_TYPE[change.type].color}`}>
                            {INFRA_CHANGE_TYPE[change.type].label}
                          </span>
                        </div>
                        <p className="text-sm text-white mt-1">{change.description || "(설명 없음)"}</p>
                      </div>
                      <div className="flex gap-1">
                        <button onClick={() => { setEditingInfra(change.id); setInfraForm({ ...change }); }} className="p-1.5 text-[#909296] hover:text-white rounded-md">
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button onClick={() => deleteInfra(change.id)} className="p-1.5 text-[#909296] hover:text-red-400 rounded-md">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                    </div>
