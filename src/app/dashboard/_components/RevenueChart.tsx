...
export function RevenueChart({ data, projectRevenues, isLoading }: RevenueChartProps) {
  ...
  const hasProjectData =
    projectRevenues && projectRevenues.some((p) => p.currentRevenue > 0 || p.targetRevenue > 0);
      ...
      {(chartView === "project" || !hasTrendData) && hasProjectData && (
        <ResponsiveContainer width="100%" height={250}>
          ...
          >
            ...
            />
            <Legend
              formatter={(value: string) => (value === "current" ? "현재 매출" : "목표 매출")}
            ...
            <Bar dataKey="current" radius={[0, 4, 4, 0]} barSize={20}>
              {projectChartData.map((entry, index) => (
                <Cell
                  key={`cell-${index}`}
                  fill={PROJECT_COLORS[entry.project as keyof typeof PROJECT_COLORS]}
                />
              ))}
            </Bar>
          </BarChart>
