...
export function GoalComparisonView({ goals, isOpen, onClose }: GoalComparisonViewProps) {
  ...
  const toggleGoalSelection = (goalId: string) => {
    setSelectedGoalIds((prev) =>
      prev.includes(goalId) ? prev.filter((id) => id !== goalId) : prev.length < 4 ? [...prev, goalId] : prev
    );
  };
  const calculateProgress = (current: string, target: string) => {
    const curr = Number(current), tgt = Number(target);
      ...
      <div className="bg-[#1a1b23] rounded-xl border border-[#373A40] w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-auto p-5">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* 목표 선택 패널 */}
            <div>
              <h3 className="text-sm font-medium text-[#909296] mb-3">비교할 목표 선택 (최대 4개)</h3>
                        ...
                        <div className="flex items-center gap-2">
                          <div
                            className={`w-5 h-5 rounded border flex items-center justify-center ${
                              isSelected ? "bg-brand-primary border-brand-primary" : "border-[#373A40]"
                            }`}
                          >
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-[#25262b] rounded-lg p-4 border border-[#373A40]">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-[#1a1b23] rounded p-3">
                          ...
                          <div className="text-lg font-semibold text-emerald-400">{stats.maxProgress.toFixed(0)}%</div>
                          <div className="text-xs text-[#909296] truncate">{stats.bestGoal?.title}</div>
                        </div>
                        <div className="bg-[#1a1b23] rounded p-3">
                          <div className="text-xs text-[#5c5f66] mb-1">최저 진행률</div>
                          <div className="text-lg font-semibold text-red-400">{stats.minProgress.toFixed(0)}%</div>
                          <div className="text-xs text-[#909296] truncate">{stats.worstGoal?.title}</div>
                        </div>
                      </div>
                    </div>
                  )}
                  <div className="bg-[#25262b] rounded-lg border border-[#373A40] overflow-hidden">
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {comparisonData.map(({ goal, progress, prediction, daysRemaining, onTrack }) => (
                          <tr key={goal.id} className="border-b border-[#373A40] last:border-b-0">
                            <td className="p-3">
                              <div className="flex items-center gap-2">
                                <span>{GOAL_CATEGORIES[goal.category].icon}</span>
                                <span className="text-white font-medium truncate max-w-[120px]">{goal.title}</span>
                              </div>
                            </td>
                            ...
                            <td className="p-3 text-center">
                              <span className={`px-2 py-0.5 rounded ${GOAL_STATUS[goal.status].bg} ${GOAL_STATUS[goal.status].color}`}>
                                {GOAL_STATUS[goal.status].label}
                              </span>
                            </td>
                            <td className="p-3 text-center">
                              <span className={daysRemaining < 0 ? "text-red-400" : daysRemaining < 7 ? "text-amber-400" : "text-white"}>
                                {daysRemaining}일
                              </span>
                            </td>
                            <td className="p-3 text-center">
                  ...
                  {comparisonData.some((d) => d.prediction) && (
                    <div className="bg-[#25262b] rounded-lg p-4 border border-[#373A40]">
                      ...
                      <div className="space-y-3">
                        {comparisonData.map(({ goal, currentDailyRate, requiredDailyRate, onTrack }) => {
                          ...
                          return (
                            <div key={goal.id}>
                              <div className="flex items-center justify-between mb-1">
                                <span className="text-xs text-white truncate max-w-[150px]">{goal.title}</span>
                                <span className={`text-xs ${onTrack ? "text-emerald-400" : "text-amber-400"}`}>
