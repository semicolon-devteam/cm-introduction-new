        ...
        {WEEKDAY_NAMES.map((day, index) => (
          <div
            ...
            className={`py-2 text-center text-xs font-medium ${
              index === 0 ? "text-red-400" : index === 6 ? "text-blue-400" : "text-[#909296]"
            }`}
          >
            {day}
          </div>
                  ...
                  {selectedEvents.map((event) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        <span
                          className={`px-2 py-0.5 text-xs rounded ${EVENT_TYPE_COLORS[event.type].bg} ${EVENT_TYPE_COLORS[event.type].text}`}
                        >
                      ...
                      {event.type === "github_issue" &&
                        ...
                        "repository" in event.metadata && (
                          <div className="space-y-1.5 text-xs text-[#909296]">
                              ...
                              <div className="flex items-center gap-1.5">
                                ...
                                {"number" in event.metadata && event.metadata.number && (
                                  <span className="text-[#5c5f66]">
                                    #{event.metadata.number as number}
                                  </span>
                                )}
                              </div>
                            )}
                            ...
                            {/* 담당자 */}
                            {"assignees" in event.metadata &&
                              Array.isArray(event.metadata.assignees) &&
                              event.metadata.assignees.length > 0 && (
                                <div className="flex items-center gap-1.5">
                                  <User className="w-3 h-3" />
                                  <span>
                                    {(event.metadata.assignees as string[])
                                      .map((a) => `@${a}`)
                                      .join(", ")}
                                  </span>
                                </div>
                            ...
                            {/* 라벨 */}
                            {"labels" in event.metadata &&
                              Array.isArray(event.metadata.labels) &&
                              event.metadata.labels.length > 0 && (
                                <div className="flex items-center gap-1.5 flex-wrap mt-2">
                                  {(event.metadata.labels as Array<{ name: string; color: string }>)
                                    .slice(0, 5)
                                    .map((label, idx) => (
                                      <span
                                        key={idx}
